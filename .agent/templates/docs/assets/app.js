...
class DocsApp {
    ...
    async renderERDiagram() {
        ...
        try {
            const response = await fetch(erList[0].path);
    ...
    async loadData() {
        try {
            const [project, modules, pending, changelog, kanban, rules, diagrams, usecases] = await Promise.all([
                ...
                fetch('data/pending.json').then(r => r.json().catch(() => ({}))), // Graceful fail
                fetch('data/changelog.json').then(r => r.json().catch(() => ({}))),
                fetch('data/kanban.json').then(r => r.json().catch(() => ({ columns: [] }))),
                fetch('data/rules.json').then(r => r.json().catch(() => ({ rules: [] }))),
                fetch('data/diagrams.json').then(r => r.json().catch(() => ({ flow: [], usecase: [] }))),
                fetch('data/usecases.json').then(r => r.json().catch(() => ({ usecases: [] })))
            ]);
    ...
    renderDiagrams(type) {
        ...
        list.forEach((diagram, index) => {
            ...
            btn.addEventListener('click', () => {
                ...
                btn.classList.add('active');
                if (renderer) renderer.loadAndRender(diagram.path);
            });
            container.appendChild(btn);

            // Auto load first
            if (index === 0 && renderer) {
                renderer.loadAndRender(diagram.path);
            }
        });
    }
